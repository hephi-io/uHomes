...
export class BookingService {
  ...
  async updateBookingStatus(bookingId: string, status: BookingStatus, userId: string): Promise<IBooking> {
    ...
    const booking = await Booking.findById(bookingId)
    if (!booking) throw new NotFoundError('Booking not found')
    if (booking.agent.toString() !== userId) throw new UnauthorizedError('Only the assigned agent can update this booking')
  ...
  async deleteBooking(bookingId: string, user: { id: string; role: string }): Promise<IBooking> {
    ...
    const canDelete = user.role === 'Admin' || booking.agent.toString() === user.id || booking.tenant.toString() === user.id
