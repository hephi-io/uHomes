...
describe('AgentService', () => {
  describe('forgotPassword', () => {
    ...
    it('should throw NotFoundError for non-existing email', async () => {
      await expect(agentService.forgotPassword('noone@example.com')).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('resetPassword', () => {
    ...
    it('should throw BadRequestError for invalid token', async () => {
      await expect(agentService.resetPassword('invalidtoken', 'newPass')).rejects.toThrow(
        BadRequestError
      );
    });
  });
  ...
  describe('resendResetToken', () => {
    ...
    it('should throw NotFoundError for non-existing agent', async () => {
      await expect(agentService.resendResetToken('unknown@example.com')).rejects.toThrow(
        NotFoundError
      );
    });
  });
...
describe('Agent CRUD Operations', () => {
  ...
  it('should get all agents', async () => {
    const res = await request(app).get('/api/agent').set('Authorization', `Bearer ${token}`);
