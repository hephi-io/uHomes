...
export function Help() {
  ...
  return (
    <>
      <h1 className="font-semibold text-2xl leading-[120%] tracking-[0%] text-black pl-8 py-5">
        Help
      </h1>
      <div className="border-t border-t-[#E4E4E4]" />
      <div className="p-8 mt-9">
        <div className="size-16 flex justify-center items-center rounded-full bg-[#DBEAFE]">
          <SVGs.QuestionMark className="text-[#3E78FF]" />
        </div>
        <h2 className="font-medium text-base leading-6 tracking-normal text-black mt-4">
          Help & Support
        </h2>
        <p className="text-base leading-6 tracking-normal text-[#727272] mt-4">
        ...
        {cards.map((card) => (
          <div
            key={card.id}
            className={`rounded-[10px] border-[0.8px] border-[#E5E7EB] p-[25px] ${card.id === 1 ? 'mt-10.5' : 'mt-6'}`}
          >
        ...
        <div className="rounded-[10px] border-[0.8px] border-[#E5E7EB] p-6 mt-10.5">
          ...
          {faqButtons.map((faqButton) => (
            <div key={faqButton.id} className={faqButton.id === 1 ? 'mt-6' : 'mt-2'}>
          ...
          >
            {accordions.map((accordion) => (
              ...
              >
                <AccordionTrigger className="font-Bricolage text-sm leading-[120%] tracking-normal text-black">
                  {accordion.label}
                </AccordionTrigger>
